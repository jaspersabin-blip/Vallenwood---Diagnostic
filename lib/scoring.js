...
function scoreGtm(inputs) {
  ...
  score += clamp(channelScore, 0, 5);

  if (includesAny(cycle, ["under 1 month"])) score += 5;
  else if (includesAny(cycle, ["1–3 months", "1-3 months"])) score += 4;
  else if (includesAny(cycle, ["3–6 months", "3-6 months"])) score += 3;
  else if (includesAny(cycle, ["6–12 months", "6-12 months"])) score += 2;
  else if (includesAny(cycle, ["12+ months"])) score += 1;

  if (includesAny(closeRate, ["40%+"])) score += 5;
  else if (includesAny(closeRate, ["25–40%", "25-40%"])) score += 4;
  else if (includesAny(closeRate, ["15–25%", "15-25%"])) score += 3;
  else if (includesAny(closeRate, ["under 15%"])) score += 1;
...
function scoreMeasurement(inputs) {
  ...
  if (includesAny(measuredBy, ["revenue"])) score += 5;
  else if (includesAny(measuredBy, ["pipeline"])) score += 4;
  else if (includesAny(measuredBy, ["brand metrics"])) score += 2;
  else if (includesAny(measuredBy, ["leads"])) score += 1;

  if (includesAny(attribution, ["yes"])) score += 5;
  else if (includesAny(attribution, ["debated"])) score += 3;
...
export function scoreDiagnostic(inputs = {}) {
  ...
  const osScore = clamp(
  Math.round((adjustedRawScore - 45) * 1.15 + 45),
  35,
  92
);
