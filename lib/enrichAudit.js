...
function safeJsonParse(text) {
  try {
    ...
    console.error("[enrichAudit] Raw response length:", text?.length);
    console.error("[enrichAudit] Raw response tail:", text?.slice(-200));
    return {};
  }
}
...
- Each SWOT quadrant MUST contain exactly 3 separate items — not 1, not 2, exactly 3. Each must be a distinct commercially-grounded observation.
- The root_cause_hypotheses array MUST contain exactly 3 items.
- The constraint_chain array MUST contain exactly 3 items.
