...
function buildHiddenReportData(report) {
  ...
  return {
    ...
    headline_diagnosis: report?.narrative?.headline_diagnosis || report?.headline_diagnosis || "",
    executive_headline: report?.narrative?.executive_headline || report?.executive_headline || "",
    the_operating_tension: report?.narrative?.the_operating_tension || report?.the_operating_tension || "",
    what_this_means_in_practice: report?.narrative?.what_this_means_in_practice || report?.what_this_means_in_practice || [],
    diagnosis_implications: report?.diagnosis_implications || [],
    operating_tensions: report?.scoring?.operating_tensions || [],
    what_good_looks_like: report?.narrative?.what_good_looks_like || report?.what_good_looks_like || "",
    upgrade_bridge: report?.narrative?.upgrade_bridge || report?.upgrade_bridge || [],
    root_cause_hypotheses: report?.full_tier?.root_cause_hypotheses || report?.root_cause_hypotheses || [],
    swot: report?.full_tier?.swot || report?.swot || null,
    constraint_chain: report?.full_tier?.constraint_chain || report?.constraint_chain || [],
    competitive_context: report?.full_tier?.competitive_context || report?.competitive_context || null,
    pricing_packaging_audit: report?.full_tier?.pricing_packaging_audit || report?.pricing_packaging_audit || null,
    roadmap: report?.full_tier?.roadmap || report?.roadmap || null,
